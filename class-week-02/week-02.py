...
def main():
    ...
    st.header('Semana 2 - Pré-Processamento de Dados em Python')

    file = st.file_uploader('Escolha a base de dados que deseja analisar (.csv)', type='csv')
    if file is not None:
        st.subheader('Analisando os dados do dataframe (5 primeiras linhas):')
        df = pd.read_csv(file)
        st.dataframe(df.head(5))

        st.markdown('**Número total de linhas:**')
        st.markdown(df.shape[0])

        st.markdown('**Número total de colunas:**')
        st.markdown(df.shape[1])

        st.markdown('**Visualização personalizada do dataframe:**')
        number = st.slider('Escolha o numero de colunas que deseja ver:', min_value=1, max_value=10)
        ...
        st.subheader('Inputaçao de dados numéricos :')
        percentual = st.slider('Limite de percentual faltante para as colunas você deseja inputar os dados',
                               min_value=0, max_value=100)
        lista_colunas = list(exploracao[exploracao['NA %'] < percentual]['nomes'])

        select_method = st.radio('Escolha um método abaixo:', ('Média', 'Mediana'))
        ...
        if select_method == 'Média':
            ...
            st.subheader('Dados inputados disponíveis (faça o download abaixo): ')
        ...
        if select_method == 'Mediana':
            ...
            st.subheader('Dados inputados disponíveis (faça download abaixo): ')
            st.markdown(get_table_download_link(df_inputado), unsafe_allow_html=True)
